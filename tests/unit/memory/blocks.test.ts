...
describe('Block Management', () => {
  ...
  describe('createUserBlockTemplate', () => {
    it('should create block with correct label format', async () => {
      const { createUserBlockTemplate } = await import('../../../src/memory/blocks');
  ...
  describe('attachBlockToAgent', () => {
    it('should return true on successful attach', async () => {
      ...
      expect(mockClient.agents.blocks.attach).toHaveBeenCalledWith('agent-1', { blockId: 'block-1' });
    });
    ...
    it('should rethrow non-409 errors', async () => {
      ...
      const mockClient = {
        agents: {
          blocks: {
            attach: vi.fn().mockRejectedValue({ status: 500, message: 'Server error' }),
          },
        },
      };

      await expect(attachBlockToAgent(mockClient, 'agent-1', 'block-1'))
  ...
  describe('detachBlockFromAgent', () => {
    it('should return true on successful detach', async () => {
      const { detachBlockFromAgent } = await import('../../../src/memory/blocks');
      ...
      const result = await detachBlockFromAgent(mockClient, 'agent-1', 'block-1');

      expect(result).toBe(true);
      expect(mockClient.agents.blocks.detach).toHaveBeenCalledWith('agent-1', 'block-1');
    });
    ...
    it('should rethrow non-404 errors', async () => {
      ...
      const mockClient = {
        agents: {
          blocks: {
            detach: vi.fn().mockRejectedValue({ status: 500, message: 'Server error' }),
          },
        },
      };

      await expect(detachBlockFromAgent(mockClient, 'agent-1', 'block-1'))
        .rejects.toEqual({ status: 500, message: 'Server error' });
    });
  });
  ...
  describe('getOrCreateUserBlock', () => {
    it('should return existing block when found', async () => {
      const { getOrCreateUserBlock } = await import('../../../src/memory/blocks');
      ...
      const result = await getOrCreateUserBlock(mockClient, 'agent-1', 'user-123');
